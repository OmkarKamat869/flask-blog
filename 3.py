...
def dashboard():
    if 'user' in session and session['user'] == params["login_user"] :
        posts=Posts.query.all()
        return render_template('dashboard.html',params=params, posts=posts)
    if (request.method== 'POST'):
        username = request.form.get('uname')
        password = request.form.get('pass')
        if(username== params["login_user"] and password== params["login_password"]):
...
def edit(sno):
   if 'user' in session and session['user'] == params["login_user"] :
       if(request.method=='POST'):
           ...
           if sno == '0':
               post = Posts(title=box_title,tag_line=tagline,slug=slug,content=content,date= datetime.now(),img_file=img_file)
               db.session.add(post)
               db.session.commit()
           else : 
